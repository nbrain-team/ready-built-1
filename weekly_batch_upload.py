#!/usr/bin/env python3
"""
Weekly batch upload - processes data week by week for optimal performance
"""
...
# Progress tracking
PROGRESS_FILE = 'weekly_upload_progress.json'
...
def get_weeks_in_file(file_path, date_column):
    """Get list of week start dates in a file"""
    print(f"  Scanning file for weeks...")
    weeks = set()
    
    # Read file in chunks to get all weeks
...
def upload_transactions_by_week(session, file_path, week_start):
    """Upload transaction data for a specific week"""
    week_end = week_start + timedelta(days=6)
    week_str = f"{week_start.strftime('%Y-%m-%d')} to {week_end.strftime('%Y-%m-%d')}"
    
    print(f"\n  Processing week {week_str}...")
    
    total_count = 0
    chunk_size = 1000  # Smaller chunks for weekly processing
    ...
    for chunk in pd.read_csv(file_path, chunksize=chunk_size):
        ...
        for _, row in chunk.iterrows():
            try:
                ...
                # Only process this specific week
                if not (week_start <= sale_date.date() <= week_end):
                    continue
...
def upload_timeclock_by_week(session, file_path, week_start):
    """Upload time clock data for a specific week"""
    week_end = week_start + timedelta(days=6)
    week_str = f"{week_start.strftime('%Y-%m-%d')} to {week_end.strftime('%Y-%m-%d')}"
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # Only process this specific week
            if not (week_start <= clock_date.date() <= week_end):
                continue
            ...
            timecard_id = f"tc_{week_id}_{idx}"
...
def upload_schedules_by_week(session, file_path, week_start):
    """Upload schedule data for a specific week"""
    week_end = week_start + timedelta(days=6)
    week_str = f"{week_start.strftime('%Y-%m-%d')} to {week_end.strftime('%Y-%m-%d')}"
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # Only process this specific week
            if not (week_start <= schedule_date.date() <= week_end):
                continue
            ...
            week_id = week_start.strftime('%Y%m%d')
            schedule_record_id = f"sched_{week_id}_{idx}"
...
def main():
    print("=" * 60)
    print("WEEKLY BATCH UPLOAD (2025 DATA ONLY)")
    print("=" * 60)
    print(f"Database: {DATABASE_URL.split('@')[1] if '@' in DATABASE_URL else 'configured'}")
    print("\n📅 Processing data week by week for optimal performance")
    print()
    ...
    try:
        ...
        if not progress:
            ...
            progress = {'weeks_completed': [], 'totals': {}}
        ...
        data_types = [
            {
                ...
                'upload_func': upload_transactions_by_week
            },
            {
                'name': 'Time Clock',
                'file': 'blazer/Time Clock Data 2025 071825.csv',
                'date_column': 'Date',
                'upload_func': upload_timeclock_by_week
            },
            {
                'name': 'Schedules',
                'file': 'blazer/Schedule Records.csv',
                'date_column': 'Schedule Date',
                'upload_func': upload_schedules_by_week
            }
        ]
        ...
        for data_type in data_types:
            ...
            # Get weeks in file
            weeks = get_weeks_in_file(data_type['file'], data_type['date_column'])
            print(f"  Found {len(weeks)} weeks of data (2025+)")
            
            # Process each week
            type_total = 0
            for week_start in weeks:
                week_key = f"{data_type['name']}_{week_start.strftime('%Y%m%d')}"
                
                # Skip if already completed
                if week_key in progress['weeks_completed']:
                    week_str = week_start.strftime('%Y-%m-%d')
                    print(f"    ✓ Week {week_str}: Already completed")
                    continue
                
                # Process this week
                count = data_type['upload_func'](session, data_type['file'], week_start)
                type_total += count
                
                # Update progress
                progress['weeks_completed'].append(week_key)
                save_progress(progress)
